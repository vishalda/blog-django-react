...
class ViewPost extends React.Component {
  ...
  render() {
    ...
    const Description = this.state.post
      ? this.state.post.description
      : "Description";
    const Image = this.state.post
      ? this.state.post.image
      : "/home/vishal/Pictures/01-28-2021-11.32.04.jpg";
    const AuthorName = this.state.post.author
      ? this.state.post.author.name
      : "Vishal";
    const AuthorUserName = this.state.post.author
      ? this.state.post.author.username
      : "VDA-001";
    const AuthorId = this.state.post.author
      ? parseInt(this.state.post.author.id)
      : 0;
    const numberOfComments = this.state.post
      ? this.state.post.number_of_comments
      : "0";
    ...
    return (
      <div className="view-post">
        ...
        <div className="sticky-top left-div large-view-div">
          <iframe
            src={`https://robohash.org/${AuthorUserName}`}
            className="left-div-image"
            title="large-view-user-profile-pic"
          ></iframe>
          <span>
          ...
          <hr />
          <p>
            <FaRegComments className="icon" />:{numberOfComments}&nbsp; &nbsp;
            <MdDateRange className="icon" />
            {CreatedAt}
          </p>
          <Button onClick={this.referenceFunction} className="button">
            Comments
          </Button>
          <br />
          <br />
          {IsAuthenticated() && AuthorId === IsAuthenticated().user.id && (
            <Button
              className="button"
              href={`/update-post/${this.state.post.id}`}
            >
              Update this Post
            </Button>
          )}
        ...
        <div className="fixed-bottom small-view-div">
          <Button
            onClick={this.referenceFunction}
            className="button comments-button"
          >
        ...
        <Modal
          ...
          className="view-comments text-dark"
        >
        ...
        <Container className="post-detail">
          ...
          <div className="small-view-user-info-div">
            <iframe
              src={`https://robohash.org/${AuthorUserName}`}
              className="left-div-image"
              title="small-view-user-profile-pic"
            ></iframe>
            <span>
              <h5>@{AuthorUserName}</h5>
              <h6>{AuthorName}</h6>
